...
def findPathCell(i, j, mat, dp): 
	...
	if (j<n-1 and ((mat[i][j] +1) == mat[i][j + 1])): 
		x = 1 + findPathCell(i, j + 1, mat, dp) 
	if (j>0 and (mat[i][j] +1 == mat[i][j-1])): 
		y = 1 + findPathCell(i, j-1, mat, dp) 
	if (i>0 and (mat[i][j] +1 == mat[i-1][j])): 
		z = 1 + findPathCell(i-1, j, mat, dp) 
	if (i<n-1 and (mat[i][j] +1 == mat[i + 1][j])): 
		w = 1 + findPathCell(i + 1, j, mat, dp) 
...
mat = [[1, 2, 9], 
	[5, 3, 8], 
	[4, 6, 7]] 
print("Length of the longest path is ", findLongestPath(mat))
```
